...
const ord = (n) => n +
    { e: "st", o: "nd", w: "rd", h: "th" }[new Intl.PluralRules("en", { type: "ordinal" }).select(parseInt(n))[2]];
...
function renderAnswer(str) {
    ...
    const process = (str, answer, i) => {
        ...
        let word = broken[0], note = broken[1];
        res.append(word);
        if (note)
    ...
    if (str instanceof Array)
        str.forEach((a, i) => process(str, a, i));
    else
        process([], str, -1);
    return res;
}
...
class Switcher {
    ...
    listen() {
        ...
        for (const trigger of this.triggers)
            trigger.addEventListener("click", () => this.showPane(trigger.dataset.pane));
        return this;
    }
    indexPanes() {
        let indexed = {};
    ...
    showPane(id) {
        ...
        if (id.startsWith("."))
            target = this.history[1];
        if (!target)
            target = this.panes["404"];
        target.classList.add("showing");
        if (this.history[0])
            this.history[0].classList.remove("showing");
        this.history.unshift(target);
        return this;
    }
}

let typeMap = ["info", "success", "error"], queue = [], showing = false, id = 0, container = createElement("div", "class:toast-container");
...
class Message {
    constructor(content, type, duration, title) {
        ...
        if (title)
            el.append(createElement("h3", "class:toast-header", title));
...
function wantToShow() {
    if (!queue.length)
        return;
    if (showing)
        return setTimeout(wantToShow, queue[0].duration);
...
class Grader {
    ...
    gradeQuestion(question, userAnswer) {
        ...
        if (question.answer.constructor === Array) {
            ...
            let directEquals = correct.some(el => purify(el) === userAnswer), fuzzyEquals = correct.some(el => this.equals(userAnswer, el));
            if (directEquals)
                return 2;
            else if (fuzzyEquals)
                return 1;
        ...
        // if there's only 1 answer
            ...
            let directEquals = userAnswer === purify(question.answer), fuzzyEquals = this.equals(userAnswer, question.answer);
            if (directEquals)
                return 2;
            else if (fuzzyEquals)
                return 1;
    ...
    equals(word1, correct) {
        ...
        for (let i = 0; i < word1.length; i++) {
            if (word1.charAt(i) === correct.charAt(i))
                continue;
            foundMistakes--;
            if (foundMistakes < 0)
                return false;
    ...
    finish(questions) {
        ...
        for (let [i, question] of questions.entries()) {
            let qSum = createElement("div", "class:quiz-results-q", `${i + 1}. ${question.question}: `), qWrong = createElement("span", "class:quiz-results-q-wrong", question.graded.userAnswer), qCorrect = createElement("span", "class:quiz-results-q-correct");
            qCorrect.append(renderAnswer(question.answer));
            if (question.graded.isCorrect > 0)
                numCorrect++;
            else
                qSum.append(qWrong);
...
class WalkthroughMan {
        ...
        this.listen = {
            next: () => {
                ...
                // done grading and expecting to go to the next question?
                    ...
                    if (!this.options.immediateGrade)
                        this.gradeQuestion();
                    // next question
    ...
    initialize(questions, options) {
        ...
        if (this.options.immediateGrade)
            this.btns.next.innerHTML = "Grade";
        // update current question indicator
        $("#count-total").textContent = this.questions.length;
        $("#count-current").textContent = "1";
        // Load the first question
        this.loadQuestion(0, 0, 1).then(() => (this.defaultHeight = this.container.getBoundingClientRect().height));
        this.updateDisable();
    ...
    loadQuestion(index, delay, n) {
        ...
        if (this.questions[index].graded)
            this.timeTo = 2;
        this.updateNextBtn();
    ...
    updateDisable() {
        ...
        if (this.curQuestionIndex === 0)
            this.btns.prev.disabled = true;
        else
            this.btns.prev.disabled = false;
    ...
    gradeQuestion() {
        ...
        if (cur.graded && !this.options.immediateGrade)
            return;
        // get grade
        ...
        if (this.options.immediateGrade)
            this.updateGrade(isCorrect, cur.answer);
...
function generateVocabHTML(questionData) {
    let title = createElement("h3", "class:quiz-question-title", `${questionData.word}${questionData.dictionary ? ", " + questionData.dictionary : ""}`), input = createElement("input", "placeholder:What's the translation? Enter...;type:text;class:quiz-question-input"), header = createElement("h4", "class:quiz-question-super", `translate the ${questionData.type}`), container = createElement("div", "class:animator-inner");
    container.append(header, title, input);
    return container;
}
...
class Formulator {
    ...
    initialize(endings, vocab) {
        ...
        for (let declnum in getFrom)
            this.questions.push(...this.questionGenerators.declensions.bind(this)(+declnum + 1, getFrom[declnum]));
...
class Initializer {
    constructor() {
        this.quizIsEmpty = () => !this.options.declensions && (!this.optEls.vocabNum.value || this.optEls.vocabNum.value === "-1");
        this.optEls = {
...
class Loader {
    constructor() {
        this.update = {
            declensions: ([{ declensions }]) => {
                ...
                genders = new Set(Array.from(genders).map((n) => map[n]));
                // show every gender
                Array.from($$(`.view-table-head, .view-table-field`)).forEach((e) => (e.style.display = "table-cell"));
    ...
    async initialize(data) {
        ...
        this.trigger.addEventListener("click", (e) => this.update[this.options.type.value].apply(this, [data]));
